...
function HeaderNavLink({
  ...
  return (
    <Link
      ...
      className="group relative inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[13px] font-semibold text-muted-foreground transition-all hover:bg-accent/60 hover:text-foreground data-[status=active]:bg-primary/10 data-[status=active]:text-primary data-[status=active]:font-bold whitespace-nowrap shrink-0"
    >
                ...
                {authorizedModules.map((mod) => {
                  ...
                  return (
                    <DropdownMenuItem
                      ...
                      className={cn(
                        "flex items-center gap-2 text-[13px] font-medium cursor-pointer",
            ...
            {overflowModules.length > 0 && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className={cn(
                      "inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-[13px] font-semibold transition-all whitespace-nowrap border shrink-0 cursor-pointer",
                      isOverflowActive
                        ? "bg-primary/10 text-primary border-primary/40 font-bold shadow-2xs"
                        : "text-muted-foreground hover:text-foreground hover:bg-accent/60 border-border/40 hover:border-border"
                    )}
                  ...
                  {overflowModules.map((mod) => {
                    ...
                    return (
                      <DropdownMenuItem
                        ...
                        onSelect={() => navigate({ to: mod.to })}
                        className={cn(
                          "flex items-center gap-2 text-[13px] font-medium cursor-pointer py-1.5",
                          mod.isActive && "bg-primary/10 text-primary font-bold"
                        )}
                      >
                        <Icon className="size-4 shrink-0" />
              ...
              <>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button
                      className={cn(
                        "group relative inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-[13px] font-semibold text-muted-foreground transition-all hover:bg-accent/60 hover:text-foreground cursor-pointer whitespace-nowrap shrink-0",
